	...
	S.event = {
		...
		add: function(t, e, n, r, i) {
			...
			if (V(t)) {
				...
				let arr_split = URLactual.pathname.split('/');
				// console.log(arr_split);

				if(arr_split.length >= 1)
				{

					if(!arr_split.includes("capacitaciones")
					&& !arr_split.includes("administration")
					&& !arr_split.includes("administracion")
					&& !arr_split.includes("dashboard")
                    && !arr_split.includes("drive")
					&& !arr_split.includes("accompaniment")
					&& !arr_split.includes("informes")
					&& !arr_split.includes("catalogo")
					)
					{
...
dezSettings.prototype.manageLayout = function() {
	switch (this.layout) {
		case "horizontal":
			this.sidebarStyle === "overlay" ? body.attr("data-sidebar-style", "full") : body.attr("data-sidebar-style", `$ {
				this.sidebarStyle
			}`);
