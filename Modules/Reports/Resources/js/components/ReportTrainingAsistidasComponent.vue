  ...
  export default {
    ...
    methods: {
      async getDataAll(url = null) {
        url = url ?? `${this.url}` + "informes/get-report-training-asistidas";
      ...
      previousPage() {
        ...
        this.getDataAll(
          `${this.url}` +
            "informes/get-report-training-asistidas?page=" +
            this.data.paginate.current_page
        );
        loading(false);
      },
      ...
      async numPage(num) {
        ...
        await this.getDataAll(
          `${this.url}` + "informes/get-report-training-asistidas?page=" + num
        );
        loading(false);
      },
      ...
      async downloadReportTraining() {
        loading();
        let rs = await fetch(
          this.url + "informes/descargar-excel-entrenamiento-asistidas",
          {
            method: "POST",
            body: JSON.stringify({
              search: this.search,
        ...
        if (rs.status == 200) {
          ...
          link.href = url;
          link.download = "Informe Asistidas por experto.xlsx";
          link.click();
